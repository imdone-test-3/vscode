	...
	public resolve(refresh?: boolean): TPromise<ITextEditorModel> {
		...
		return this.modelReference.then(ref => {
			...
			if (!(model instanceof ResourceEditorModel)) {
				...
				return TPromise.wrapError<ITextEditorModel>(new Error(`Unexpected model for ResourceInput: ${this.resource}`)); // TODO @Ben eventually also files should be supported, but we guard due to the dangerous dispose of the model in dispose() id:145 gh:146
			}
